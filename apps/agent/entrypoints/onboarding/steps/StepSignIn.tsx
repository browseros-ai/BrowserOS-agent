...
export const StepSignIn = ({ direction, onContinue }: StepSignInProps) => {
  const [email, setEmail] = useState('')
  const [state, setState] = useState<SignInState>('idle')
  const [error, setError] = useState<string | null>(null)

  const handleSkip = async () => {
    await onboardingSignedInStorage.setValue(false)
  ...
  const handleMagicLink = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await onboardingSignedInStorage.setValue(true)
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Redirect back to step 4 after OAuth completes
      await authRedirectPathStorage.setValue('/onboarding/steps/4')
      await signIn.social({
        provider: 'google',
        callbackURL: '/home',
      })
        ...
        <div className="w-full max-w-md space-y-6">
          <div className="space-y-2 text-center">
            ...
            <p className="text-base text-muted-foreground">
              Unlock app connections and cloud features
            </p>
          </div>
