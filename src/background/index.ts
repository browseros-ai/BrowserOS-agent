...
const tabPanelState = new Map<
  number,
  { isOpen: boolean; isToggling: boolean }
>();
...
function registerHandlers(): void {
  ...
  messageRouter.registerHandler(
    ...
    (msg, port, execId) =>
      executionHandler.handleExecuteQuery(msg, port, execId),
  );

  messageRouter.registerHandler(MessageType.CANCEL_TASK, (msg, port, execId) =>
    executionHandler.handleCancelTask(msg, port, execId),
  );

  messageRouter.registerHandler(
    MessageType.RESET_CONVERSATION,
    (msg, port, execId) =>
      executionHandler.handleResetConversation(msg, port, execId),
  );

  messageRouter.registerHandler(
    MessageType.HUMAN_INPUT_RESPONSE,
    (msg, port, execId) =>
      executionHandler.handleHumanInputResponse(msg, port, execId),
  );

  // Provider handlers
  messageRouter.registerHandler(MessageType.GET_LLM_PROVIDERS, (msg, port) =>
    providersHandler.handleGetProviders(msg, port),
  );

  messageRouter.registerHandler(MessageType.SAVE_LLM_PROVIDERS, (msg, port) =>
    providersHandler.handleSaveProviders(msg, port),
  );

  // MCP handlers
  messageRouter.registerHandler(MessageType.GET_MCP_SERVERS, (msg, port) =>
    mcpHandler.handleGetMCPServers(msg, port),
  );

  messageRouter.registerHandler(MessageType.CONNECT_MCP_SERVER, (msg, port) =>
    mcpHandler.handleConnectMCPServer(msg, port),
  );
  ...
  messageRouter.registerHandler(MessageType.CALL_MCP_TOOL, (msg, port) =>
    mcpHandler.handleCallMCPTool(msg, port),
  );

  messageRouter.registerHandler(MessageType.MCP_INSTALL_SERVER, (msg, port) =>
    mcpHandler.handleInstallServer(msg, port),
  );

  messageRouter.registerHandler(MessageType.MCP_DELETE_SERVER, (msg, port) =>
    mcpHandler.handleDeleteServer(msg, port),
  );
  ...
  messageRouter.registerHandler(MessageType.UPDATE_PLAN, (msg, port, execId) =>
    planHandler.handleUpdatePlan(msg, port, execId),
  );
  ...
  // Log handler
  messageRouter.registerHandler(MessageType.LOG_MESSAGE, (msg, port) => {
    const logMsg = msg.payload as any;
    Logging.log(
      logMsg.source || "Unknown",
      logMsg.message,
      logMsg.level || "info",
    );
  });
  ...
  // Metrics handler
  messageRouter.registerHandler(MessageType.LOG_METRIC, (msg, port) => {
  ...
  messageRouter.registerHandler(MessageType.HEARTBEAT, (msg, port) => {
  ...
  // Panel close handler
  messageRouter.registerHandler(MessageType.CLOSE_PANEL, async (msg, port) => {
...
function handlePortConnection(port: chrome.runtime.Port): void {
  ...
  if (portInfo.type === "sidepanel" && portInfo.tabId) {
    ...
    Logging.log(
      "Background",
      `Side panel connected for tab ${tabId} with port name: ${port.name}`,
    );
    ...
    Logging.log(
      "Background",
      `Side panel connected but no tab ID in port name: ${port.name}`,
    );
  }
...
async function toggleSidePanel(tabId: number): Promise<void> {
  ...
  try {
    ...
    Logging.log(
      "Background",
      `Error toggling side panel for tab ${tabId}: ${error}`,
      "error",
    );
    ...
    if (!state.isOpen) {
      try {
        ...
        Logging.log(
          "Background",
          `Fallback failed for tab ${tabId}: ${fallbackError}`,
          "error",
        );
      }
    }
...
function initialize(): void {
  ...
  chrome.commands.onCommand.addListener(async (command) => {
    if (command === "toggle-panel") {
      Logging.log(
        "Background",
        "Toggle panel shortcut triggered (Cmd+E/Ctrl+E)",
      );
  ...
  chrome.tabs.onRemoved.addListener(async (tabId) => {
    ...
    Logging.log(
      "Background",
      `Tab ${tabId} removed, cleaned up panel state and execution`,
    );
  });
