...
} from "@/newtab/components/agents/AgentEditor";
...
export function CreateAgentPage({ onBack }: CreateAgentPageProps) {
  ...
  const [waitingForNewAgent, setWaitingForNewAgent] = useState<string | null>(
    null,
  );
              ...
              {mode === "editor" && (
                <>
                  ...
                  <span className="font-medium">
                    {activeAgentId ? displayTitle : "New"}
                  </span>
                </>
              )}
            </div>
              ...
              <aside className="w-[420px] max-w-[50vw] border-l border-border bg-background">
                <PlanGenerator
                  ...
                  onReplacePlan={(plan: {
                    ...
                    editorRef.current?.applyPlan(
                      { goal: plan.goal, steps: plan.steps },
                      { save: true },
                    );
                  }}
