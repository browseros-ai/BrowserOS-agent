...
export function PlanGenerator({
  ...
  const sendPortMessage = (
    ...
    try {
      ...
      const port = chrome.runtime.connect({
        name: `${PortPrefix.NEWTAB}:${executionId}`,
      });
      const id = crypto.randomUUID();
      const handler = (msg: any): void => {
        if (
          msg?.type === MessageType.PLAN_GENERATION_UPDATE &&
          msg?.id === id
        ) {
          onMessage(msg);
        }
    ...
    if (hasPlan && inputText.trim()) {
      ...
      sendPortMessage(
        ...
        (msg) => {
          ...
          if (status === "started" || status === "thinking")
            setAiStatus(msg.payload.content || status);
          if (status === "done") {
            setAiStatus("Done");
            setAiSteps(steps || []);
            setAiGoal(goal || "");
  ...
  return (
    <div
      className={"h-full flex flex-col bg-background-alt " + (className || "")}
    >
      {/* Header */}
        ...
        <div className="border-t border-border p-4">
          <div className="space-y-3">
            <textarea
              ...
              placeholder={
                currentPlan?.goal
                  ? "Describe how to improve this plan..."
            ...
            <button
              ...
              disabled={
                isGenerating || (!inputText.trim() && !currentPlan?.goal)
              }
            ...
            >
              {isGenerating ? (
                <span className="flex items-center justify-center gap-2">
                  ...
                  {currentPlan?.goal && inputText.trim()
                    ? "Refining Plan..."
                    : "Generating Plan..."}
                </span>
              ) : currentPlan?.goal && inputText.trim() ? (
