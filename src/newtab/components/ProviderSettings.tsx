...
const ALL_PROVIDERS: Provider[] = [
  ...
  { id: "claude", name: "Claude", icon: "/assets/new_tab_search/claude.svg" },
  {
    ...
    icon: "/assets/new_tab_search/browseros.svg",
  },
];
...
export function ProviderSettings() {
  ...
  const [draggedFrom, setDraggedFrom] = useState<"enabled" | "disabled" | null>(
    null,
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-muted-foreground">
        Drag providers to reorder or move between enabled/disabled
      </p>

      {/* Enabled Providers */}
      <div>
        <label className="text-sm font-medium mb-2 block">
          Visible in search dropdown
        </label>
        <div
            ...
            enabled.map((provider, index) => (
              <div
                ...
                className={`flex items-center gap-2 p-2 rounded hover:bg-accent cursor-move transition-all ${
                  draggedOverIndex === index && draggedFrom === "enabled"
            ...
            disabled.map((provider, index) => (
              <div
                ...
                className={`flex items-center gap-2 p-2 rounded hover:bg-accent cursor-move opacity-60 transition-all ${
                  draggedOverIndex === index && draggedFrom === "disabled"
                    ? "border-t-2 border-primary"
                    : ""
                }`}
              >
