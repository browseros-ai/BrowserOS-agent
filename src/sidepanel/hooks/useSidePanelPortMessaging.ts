...
export function useSidePanelPortMessaging() {
    ...
    const initializeConnection = async () => {
      try {
        ...
        if (activeTab?.id) {
          ...
          if (!success) {
            console.error(
              `[SidePanelPortMessaging] Failed to connect with executionId: ${tabExecutionId}`,
            );
          } else {
            console.log(
              `[SidePanelPortMessaging] Connected with executionId: ${tabExecutionId}`,
            );
          }
        } else {
        ...
        console.error(
          "[SidePanelPortMessaging] Error getting tab info:",
          error,
        );
      }
    };
  ...
  const sendMessage = useCallback(
    <T>(type: MessageType, payload: T, messageId?: string): boolean => {
      return (
        messagingRef.current?.sendMessage(type, payload, messageId) ?? false
      );
    },
    [],
  );
  ...
  const reconnect = useCallback(async (newExecutionId: string) => {
    ...
    console.log(
      `[SidePanelPortMessaging] Reconnecting with new executionId: ${newExecutionId}`,
    );
    ...
    try {
      ...
      if (!success) {
        console.error(
          `[SidePanelPortMessaging] Failed to reconnect with executionId: ${newExecutionId}`,
        );
      } else {
        console.log(
          `[SidePanelPortMessaging] Successfully reconnected with executionId: ${newExecutionId}`,
        );
      }
    } finally {
