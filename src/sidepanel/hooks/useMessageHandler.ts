...
export function useMessageHandler() {
  ...
  const reconnectCallbackRef = useRef<((executionId: string) => void) | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const handleRuntimeMessage = (message: any) => {
      if (message?.type === MessageType.SWITCH_EXECUTION_CONTEXT) {
        ...
        console.log(
          `[SidePanel] Received SWITCH_EXECUTION_CONTEXT: ${newExecutionId}, current: ${executionIdRef.current}`,
        );
  ...
  const setReconnectCallback = useCallback(
