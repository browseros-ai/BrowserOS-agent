...
export function ProviderSettingsCard(): JSX.Element | null {
  ...
  const [missing, setMissing] = useState<{
    ...
    modelId: boolean;
    type: boolean;
  }>({ name: false, apiKey: false, modelId: false, type: false });

  const isDraftSatisfied =
    !!draft &&
    !!draft.type &&
    !!draft.name &&
  ...
  const onSelectProvider = (providerId: string) => {
    ...
    try {
      ...
      const ok = sendMessage<BrowserOSProvidersConfig>(
        MessageType.SAVE_LLM_PROVIDERS as any,
        nextConfig,
      );
  ...
  const toProvider = (
    ...
    const contextWindow: number | undefined =
      p.contextWindow === undefined || p.contextWindow === ""
        ? undefined
        : typeof p.contextWindow === "string"
          ? parseInt(p.contextWindow, 10)
          : p.contextWindow;
    ...
    try {
      ...
      if (editingProviderId) {
        ...
        const updated = toProvider(
          draft,
          config.defaultProviderId === draft.id,
        );
        nextProviders = config.providers.map((p) =>
          p.id === editingProviderId
            ? { ...updated, isBuiltIn: p.isBuiltIn }
            : p,
        );
      } else {
        // Add new provider
  ...
  const onDeleteProvider = (providerId: string) => {
    ...
    const nextProviders = remaining.map((p) => ({
      ...p,
          ...
          <div className="flex items-center gap-2">
            {!draftOpen ? (
              <Button
                ...
                size="sm"
                variant="outline"
                className="h-7 px-2 rounded-full text-xs shrink-0 border-sky-400 text-sky-400 hover:text-sky-500"
              >
                Add provider
              </Button>
            ) : (
          ...
          <div className="mt-3 grid grid-cols-2 gap-3">
            <div className="flex flex-col">
              ...
              <select
                ...
                onChange={(e) =>
                  setDraft({
                    ...draft,
                    type: e.target.value as BrowserOSProviderType,
                  })
                }
              >
                {[
            ...
            <div className="col-span-2">
              ...
              <label className="flex items-center gap-2 text-xs font-medium mb-2">
                <input
                  ...
                  onChange={(e) =>
                    setDraft({ ...draft, supportsImages: e.target.checked })
                  }
                />
                Supports Images
              </label>
            ...
            <div className="col-span-2 flex justify-end mt-2">
              <Button
                onClick={onSave}
                size="sm"
                disabled={!isDraftSatisfied || saving}
              >
                Save
              </Button>
            </div>
