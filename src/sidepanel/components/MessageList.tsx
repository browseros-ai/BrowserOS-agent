...
export function MessageList({
  ...
  const {
    ...
    isUserScrolling,
    scrollToBottom,
  } = useAutoScroll<HTMLDivElement>([messages], externalContainerRef);
  ...
  const { processedBlocks } = useMemo(() => {
    const blocks: Array<{
      type: "message" | "narration-group" | "collapsed-thoughts";
      messages: Message[];
    }> = [];
    ...
    messages.forEach((message) => {
      ...
      if (isAssistant) {
        ...
        if (allNarrations.length > 0) {
          blocks.push({
            type: "collapsed-thoughts",
            messages: [...allNarrations],
          });
  ...
  const currentlyExecutingNarration = useMemo(() => {
    ...
    const lastNarrationIndex = messages.findLastIndex(
      (m) => m.role === "narration",
    );
  ...
  const _getRandomExample = useCallback(
    (count: number = 1): string[] => {
      const result: string[] = [];
              ...
              {currentExamples.map((prompt, index) => (
                <div
                  ...
                  className={`relative w-full transition-all duration-500 ease-in-out ${
                    isAnimating && index === 0
                      ? "animate-fly-in-top"
                      : isAnimating && index === currentExamples.length - 1
                        ? "animate-fly-out-bottom"
                        : ""
                  }`}
                >
                  <Button
        ...
        <div className="p-6 space-y-3 pb-4">
          {(() => {
            ...
            while (i < processedBlocks.length) {
              ...
              if (
                block.type === "collapsed-thoughts" &&
                nextBlock?.type === "narration-group"
              ) {
                    ...
                    <div className="relative">
                      ...
                      {nextBlock.messages.map((message, index) => {
                        const isCurrentlyExecuting =
                          message.msgId === currentlyExecutingNarration;
                        const isNewMessage = newMessageIdsRef.current.has(
                          message.msgId,
                        );
              ...
              // Standalone narration-group (when <= 3 narrations)
                ...
                renderedBlocks.push(
                  <div key={`narration-group-${i}`} className="relative">
                ...
                const isTodoTable = message.content.includes(
                  "| # | Status | Task |",
                );
                const isThinking = message.role === "thinking";
                ...
                renderedBlocks.push(
                  <div
                    ...
                    style={{
                      animationDelay: isNewMessage ? "0.1s" : undefined,
                    }}
                  >
                    {shouldIndent ? (
