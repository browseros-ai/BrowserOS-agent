...
export function ChatInput({ isConnected, isProcessing }: ChatInputProps) {
  ...
  const { getContextTabs, toggleTabSelection, clearSelectedTabs } =
    useTabsStore();
  ...
  useEffect(() => {
    ...
    return () =>
      removeMessageListener<any>(MessageType.WORKFLOW_STATUS, handleWorkflow);
  ...
  const submitTask = (query: string) => {
    ...
    const tabIds =
      contextTabs.length > 0 ? contextTabs.map((tab) => tab.id) : undefined;
  ...
  const getPlaceholder = () => {
    ...
    return chatMode
      ? "Ask about this page..."
      : "Ask me anything... (/ to pick an agent)";
  };
        ...
        >
          <div className="relative flex items-end w-full transition-all duration-300 ease-out">
            ...
            <div className="relative flex-1">
              ...
              >
                <img
                  src="assets/arrow_upward_alt.svg"
                  alt=""
                  aria-hidden="true"
                  className="w-6 h-6 block pointer-events-none select-none"
                />
              </Button>
            </div>
          </div>
