...
describe("TodoStore-unit-test", () => {
  ...
  it("tests that only one todo can be marked as doing at a time", () => {
    ...
    expect(() => store.markDoing(2)).toThrow(
      "Cannot mark TODO 2 as doing - TODO 1 is already in progress",
    );
...
describe("TodoStore-integration-test", () => {
  ...
  it.skipIf(
    ...
    async () => {
      ...
      expect(todos).toHaveLength(3);
      expect(
        todos.find((t) => t.content === "Navigate to website")?.status,
      ).toBe("done");
      expect(
        todos.find((t) => t.content === "Click login button"),
      ).toBeUndefined();
