...
export class ExecutionManager {
  ...
  create(
    executionId: string,
    options: Omit<ExecutionOptions, "executionId">,
  ): Execution {
    ...
    Logging.log(
      "ExecutionManager",
      `Created execution ${executionId} (total: ${this.executions.size})`,
    );
  ...
  async delete(executionId: string): Promise<void> {
    ...
    if (!execution) {
      Logging.log(
        "ExecutionManager",
        `Execution ${executionId} not found for deletion`,
      );
      return;
    }
  ...
  reset(executionId: string): void {
    ...
    if (execution) {
      ...
      Logging.log(
        "ExecutionManager",
        `Execution ${executionId} not found for reset`,
      );
    }
  }
  ...
  private async _disposeExecution(executionId: string): Promise<void> {
    ...
    Logging.log(
      "ExecutionManager",
      `Disposed execution ${executionId} (remaining: ${this.executions.size})`,
    );
  }
}
