...
describe("ChatAgent", () => {
  ...
  it("tests that getCurrentTabIds uses correct source based on selection", async () => {
    ...
    browserContext.getCurrentPage = vi
      .fn()
  ...
  it("tests that browser state messages are replaced correctly", () => {
    ...
    expect(
      messages.some((m) =>
        m.content?.includes("<BrowserState>First page content</BrowserState>"),
      ),
    ).toBe(true);
    ...
    expect(
      messages.some((m) =>
        m.content?.includes("<BrowserState>Second page content</BrowserState>"),
      ),
    ).toBe(true);
    expect(
      messages.some((m) => m.content?.includes("First page content")),
    ).toBe(false);

    // Should only have one browser state message
    const browserStateCount = messages.filter(
      (m) =>
        m.content?.includes("<BrowserState>") &&
        m.content?.includes("</BrowserState>"),
    ).length;
