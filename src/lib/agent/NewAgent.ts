...
export class NewAgent {
  ...
  private async _run(task: string): Promise<void> {
    ...
    while (!done && plannerIterations < MAX_PLANNER_ITERATIONS) {
      ...
      // Emit observation and challenges to UI
      const planSummary = [
        plan.observation,
        plan.challenges ? `Challenges: ${plan.challenges}` : null,
        plan.reasoning,
      ]
        .filter(Boolean)
        .join("\n\n");

      this.pubsub.publishMessage(PubSub.createMessage(planSummary, "thinking"));

      // Add planner context to message manager
...
Based on what you observe, what should we do next?`;
      ...
      // Queue screenshot and browser state for message manager
      if (screenshot) {
        this.messageManager.queueScreenshot(screenshot);
      }
      ...
      this.messageManager.queueBrowserState(JSON.stringify(browserState));
  ...
  private async _runExecutor(actions: string[]): Promise<ExecutorResult> {
    ...
    const initialInstruction = `Execute these actions in sequence:
${actions.map((action: string, i: number) => `${i + 1}. ${action}`).join("\n")}

REMEMBER: 
Use the appropriate tools to accomplish each action.
You can create a todo list to track your progress if needed.
Call done once completed.`;
  ...
  private _cleanup(): void {
    this.iterations = 0;
    Logging.log("NewAgent", "Cleanup complete", "debug");
  }
}
