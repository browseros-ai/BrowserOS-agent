...
describe("BrowserAgent-unit-test", () => {
  ...
  it("tests that errors are handled gracefully", async () => {
    ...
    vi.spyOn(browserAgent as any, "_classifyTask").mockRejectedValue(
      new Error("Classification failed"),
    );

    // Execute should throw error
    await expect(browserAgent.execute("test task")).rejects.toThrow(
      "Classification failed",
    );

    // Verify error was emitted with the wrapped error message
    expect(errorSpy).toHaveBeenCalledWith(
      "Oops! Got a fatal error when executing task: Classification failed",
      true,
    );
  });
      ...
      steps: [
        {
          ...
          reasoning: "Need to go to the site first",
        },
        {
          action: "Find search box",
          reasoning: "Need to locate search functionality",
        },
...
describe("BrowserAgent-integration-test", () => {
  ...
  it.skipIf(
    ...
    async () => {
      ...
      const simpleStrategySpy = vi.spyOn(
        browserAgent as any,
        "_executeSimpleTaskStrategy",
      );
      const complexStrategySpy = vi.spyOn(
        browserAgent as any,
        "_executeMultiStepStrategy",
      );
  ...
  it.skipIf(
    ...
    async () => {
      ...
      // Start execution (don't await)
      browserAgent
        .execute("go to amazon and order toothpaste")
        .catch((error) => {
          // Do nothing
        });
  ...
  it.skipIf(
    ...
    async () => {
      ...
      browserAgent
        .execute(
          "research top 3 programming languages in 2024 and create a comparison table",
        )
        .catch((error) => {
      ...
      const hasComplexTaskFlow = messages.some(
        (msg) =>
          msg.content &&
  ...
  it.skipIf(
    ...
    async () => {
      ...
      const updateTodosSpy = vi.spyOn(
        browserAgent as any,
        "_updateTodosFromPlan",
      );
      const plannerSpy = vi.spyOn(browserAgent as any, "_createMultiStepPlan");

      // Start execution of a complex task (don't await)
      browserAgent
        .execute("find information about TypeScript and create a summary")
        .catch((error) => {
          // Expected - we'll abort early
        });
      ...
      console.log(
        "TODO contents:",
        todos.map((t) => `${t.id}: ${t.content}`).join("\n"),
      );

      // 4. Verify TODOs match plan steps pattern
      const allTodosHaveContent = todos.every(
        (todo) => todo.content && todo.content.length > 0,
      );
