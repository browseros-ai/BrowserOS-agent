...
export class BrowserAgent {
  ...
  private static readonly MAX_STEPS_FOR_COMPLEX_TASKS =
  ...
  async execute(task: string, metadata?: ExecutionMetadata): Promise<void> {
    try {
      ...
      if (metadata?.executionMode === "predefined" && metadata.predefinedPlan) {
        ...
        this.pubsub.publishMessage(
          PubSub.createMessage(
            `Executing agent: ${predefined.name || "Custom Agent"}`,
            "thinking",
          ),
        );
        ...
        if (predefined.goal) {
          this.messageManager.addHuman(
            `User's goal is: ${predefined.goal} and this is the task: ${task}`,
          );
        }
        ...
        metadata?.executionMode === "dynamic" &&
        metadata?.source === "newtab"
      ) {
        // For tasks initiated from new tab, show the startup message with task
        this.pubsub.publishMessage(
          PubSub.createMessage(`Executing task: ${task}`, "thinking"),
        );
      }
      ...
      let message: string;
      if (
        classification.is_followup_task &&
        this.messageManager.getMessages().length > 0
      ) {
  ...
  private async _executeSimpleTaskStrategy(task: string): Promise<void> {
    ...
    for (
      let attempt = 1;
      attempt <= BrowserAgent.MAX_STEPS_FOR_SIMPLE_TASKS;
      attempt++
    ) {
      ...
      if (this._detectLoop()) {
        const loopMessage =
          "Detected repetitive behavior. Breaking out of potential infinite loop.";
      ...
      if (turnResult.requiresHumanInput) {
        ...
        if (humanResponse === "abort") {
          ...
          this.pubsub.publishMessage(
            PubSub.createMessage("❌ Task aborted by human", "assistant"),
          );
        ...
        this.pubsub.publishMessage(
          PubSub.createMessage(
            "✅ Human completed manual action. Continuing...",
            "thinking",
          ),
        );
        this.messageManager.addAI(
          "Human has completed the requested manual action. Continuing with the task.",
        );
    ...
    throw new Error(
      `Task failed to complete after ${BrowserAgent.MAX_STEPS_FOR_SIMPLE_TASKS} attempts.`,
    );
  }
  ...
  // ===================================================================
  private async _executeMultiStepStrategy(
    task: string,
    initialPlan?: Plan,
  ): Promise<void> {
    ...
    while (outer_loop_index < BrowserAgent.MAX_STEPS_OUTER_LOOP) {
      ...
      if (outer_loop_index === 0 && initialPlan) {
        ...
        this.pubsub.publishMessage(
          PubSub.createMessage(
            `Using predefined plan with ${initialPlan.steps.length} steps`,
            "thinking",
          ),
        );
      } else {
      ...
      while (
        inner_loop_index < BrowserAgent.MAX_STEPS_INNER_LOOP &&
        currentTodos.includes("- [ ]")
      ) {
        this.checkIfAborted();
        ...
        if (turnResult.requiresHumanInput) {
          ...
          this.pubsub.publishMessage(
            PubSub.createMessage(
              "✅ Human completed manual action. Re-planning...",
              "thinking",
            ),
          );
          this.messageManager.addAI(
    ...
    throw new Error(
      `Task did not complete within ${BrowserAgent.MAX_STEPS_OUTER_LOOP} planning cycles.`,
    );
  }
  ...
  private async _generateTaskResult(task: string): Promise<void> {
    ...
    try {
      ...
      if (parsedResult.ok && parsedResult.output) {
        ...
        // Fallback on error
        this.pubsub.publishMessage(
          PubSub.createMessage("Task completed.", "assistant"),
        );
      }
    } catch (error) {
      // Fallback on error
    ...
    for (const msg of recentMessages) {
      if (msg) {
        ...
        if (count + 1 >= threshold) {
          console.warn(
            `Loop detected: Message "${msg.substring(0, 50)}..." repeated ${count + 1} times`,
          );
          return true;
        }
      }
    ...
    try {
      ...
      while (!this.executionContext.shouldAbort()) {
        ...
        if (Date.now() - startTime > BrowserAgent.HUMAN_INPUT_TIMEOUT) {
          this.pubsub.publishMessage(
            PubSub.createMessage(
              "⏱️ Human input timed out after 10 minutes",
              "error",
            ),
          );
          return "timeout";
        ...
        await new Promise((resolve) =>
          setTimeout(resolve, BrowserAgent.HUMAN_INPUT_CHECK_INTERVAL),
        );
      }
