...
import {
  ...
  generatePageContextMessage,
  generateTaskPrompt,
} from "./ChatAgent.prompt";
...
export class ChatAgent {
  ...
  private _registerTools(): void {
    ...
    Logging.log(
      "ChatAgent",
      `Registered ${this.toolManager.getAll().length} tools for Q&A mode`,
    );
  }
  ...
  async execute(query: string): Promise<void> {
    try {
      ...
      const needsExtraction =
        isFreshConversation || this._hasTabsChanged(currentTabIds);
      ...
      if (isUserCancellation) {
        ...
        this.pubsub.publishMessage(
          PubSub.createMessage(`Error: ${errorMessage}`, "error"),
        );
        throw error;
      }
    }
  ...
  private async _getCurrentTabIds(): Promise<Set<number>> {
    ...
    try {
      ...
      Logging.log(
        "ChatAgent",
        `Failed to get current page, using ExecutionContext: ${error}`,
        "warning",
      );
      return new Set(selectedTabIds || []);
    }
  ...
  private async _extractPageContext(): Promise<ExtractedPageContext> {
    ...
    const selectedTabIds = this.executionContext.getSelectedTabIds();
    const hasUserSelectedTabs = Boolean(
    ...
    const tabs = await Promise.all(
      pages.map(async (page) => {
        ...
        const textSections = (textSnapshot.sections || [])
          .map((section: any) => {
            if (
              section?.textResult?.text &&
              typeof section.textResult.text === "string"
            )
              return section.textResult.text;
        ...
        if (ChatAgent.INCLUDE_LINKS) {
          ...
          const linkLines = (linksSnapshot.sections || [])
            .flatMap((section: any) =>
              Array.isArray(section?.linksResult?.links)
                ? section.linksResult.links
                : [],
            )
            .map((link: any) => {
              ...
              const parts = [label, url].filter(
                (p: string) => p && p.length > 0,
              );
              return parts.join(" - ");
            })
            .map((s: string) => s.trim())
  ...
  private async _streamLLM(opts: { tools: boolean }): Promise<void> {
    ...
    for await (const chunk of stream) {
      ...
      if (chunk.content) {
        ...
        // Stream chunk to UI
        this.pubsub.publishMessage(
          PubSub.createMessageWithId(streamMsgId, fullContent, "assistant"),
        );
      }
    }
