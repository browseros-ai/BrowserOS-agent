...
export class PlanGeneratorService {
  async generatePlan(
    input: string,
    opts?: { context?: string; maxSteps?: number; onUpdate?: UpdateFn },
    ...
    try {
      onUpdate?.({
        status: "thinking",
        content: "Summarizing goal and agent name…",
      });
      const meta = await this._generateGoalAndName(
        executionContext,
        input,
        plan,
      );
      ...
      Logging.log(
        "PlanGeneratorService",
        `Meta generation failed: ${String(e)}`,
        "warning",
      );
    }
    ...
    Logging.log(
      "PlanGeneratorService",
      `Generated plan with ${plan.steps?.length || 0} steps`,
      "info",
    );
  ...
  async refinePlan(
    ...
    feedback: string,
    opts?: { maxSteps?: number; onUpdate?: UpdateFn },
  ): Promise<StructuredPlan> {
    const maxSteps = opts?.maxSteps ?? MAX_PLANNER_STEPS;
    const onUpdate = opts?.onUpdate;
    ...
    onUpdate?.({
      status: "thinking",
      content: "Calling PlannerTool for refinement…",
    });

    const raw = await plannerTool.func({
      task: currentPlan.goal
        ? `Refine plan for: ${currentPlan.goal}`
    ...
    try {
      onUpdate?.({
        status: "thinking",
        content: "Updating goal and agent name…",
      });
      const meta = await this._generateGoalAndName(
        executionContext,
        currentPlan.goal || "Refined agent",
        plan,
      );
      plan.goal = meta.goal;
      plan.name = meta.name;
      ...
      Logging.log(
        "PlanGeneratorService",
        `Meta generation failed (refine): ${String(e)}`,
        "warning",
      );
    }
    ...
    Logging.log(
      "PlanGeneratorService",
      `Refined plan with ${plan.steps?.length || 0} steps`,
      "info",
    );
  ...
  private async _generateGoalAndName(
    executionContext: ExecutionContext,
    input: string,
    plan: StructuredPlan,
  ): Promise<z.infer<typeof AgentMetaSchema>> {
    ...
    Logging.log(
      "PlanGeneratorService",
      `Generating goal/name with ${TokenCounter.format(tokenCount)}`,
      "info",
    );

    const structuredLLM = llm.withStructuredOutput(AgentMetaSchema);
    const meta = await invokeWithRetry<z.infer<typeof AgentMetaSchema>>(
      structuredLLM,
      messages,
      3,
    );
