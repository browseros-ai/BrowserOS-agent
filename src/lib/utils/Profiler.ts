  ...
  try {
    ...
    if (startTime !== undefined) {
      ...
      if (ENABLE_CONSOLE_LOGGING) {
        ...
        console.log(
          `${indent}${color} [END] ${label}: ${duration.toFixed(2)}ms`,
        );
      }
...
export function showProfileReport(): void {
  ...
  sortedMeasures.forEach((measure) => {
    ...
    console.log(
      `${color} ${measure.name.padEnd(40)} ${duration.toFixed(2).padStart(10)}ms${countInfo}`,
    );
  });
...
export function getPerformanceSummary(): {
  ...
  measures.forEach((measure) => {
    ...
    operationStats[measure.name].maxDuration = Math.max(
      operationStats[measure.name].maxDuration,
      measure.duration,
    );
  });
  ...
  const operationsArray = Object.entries(operationStats).map(
    ([name, stats]) => ({
      name,
      count: stats.count,
...
export function showPerformanceSummary(): void {
  ...
  summary.topOperations.forEach((op, index) => {
    ...
    console.log(
      `${color} ${(index + 1).toString().padEnd(2)}. ${op.name.padEnd(40)} Max: ${op.duration.toFixed(2).padStart(10)}ms | Count: ${op.count.toString().padStart(3)} | Avg: ${op.avgDuration.toFixed(2).padStart(10)}ms`,
    );
  });
  ...
  summary.frequentOperations.forEach((op, index) => {
    const color =
      op.avgDuration > 1000 ? "🔴" : op.avgDuration > 500 ? "🟡" : "🟢";
    console.log(
      `${color} ${(index + 1).toString().padEnd(2)}. ${op.name.padEnd(40)} Count: ${op.count.toString().padStart(3)} | Total: ${op.totalDuration.toFixed(2).padStart(10)}ms | Avg: ${op.avgDuration.toFixed(2).padStart(10)}ms`,
    );
  });
...
if (config.DEV_MODE) {
  ...
  if (ENABLE_CONSOLE_LOGGING) {
    ...
    console.log(
      "  profiler.exportTrace() - Export trace for Perfetto/chrome://tracing",
    );
    console.log(
      "  profiler.exportTraceLegacy() - Export trace in array format",
    );
