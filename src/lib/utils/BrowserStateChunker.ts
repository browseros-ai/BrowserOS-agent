...
export class BrowserStateChunker {
  ...
  constructor(browserStateString: string, maxTokensPerChunk: number = 25000) {
    ...
    const availableForElements =
      maxTokensPerChunk - headerTokens - bufferTokens;
    ...
    for (const line of elementLines) {
      ...
      if (
        currentTokens + lineTokens > availableForElements &&
        currentLines.length > 0
      ) {
        // Save current chunk
        this.chunks.push(
          this._buildChunk(header, currentLines, this.chunks.length),
        );
        currentLines = [];
        currentTokens = 0;
      }
