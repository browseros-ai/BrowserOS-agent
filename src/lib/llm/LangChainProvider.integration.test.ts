...
describe("LangChainProvider Integration Test", () => {
  ...
  it.skipIf(
    ...
    async () => {
      ...
      const response = await llm.invoke(
        'Say "Hello from Nxtscape!" and nothing else.',
      );
  ...
  it("should test getLLM with withStructuredOutput (mimics tools)", async () => {
    ...
    if (
      ...
      process.env.LITELLM_API_KEY !== "nokey"
    ) {
      const systemPrompt =
        "You are a helpful assistant that creates action plans.";
      const userPrompt =
        "Create a simple 2-step plan to make coffee. Respond with JSON.";
