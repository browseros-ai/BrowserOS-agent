...
export class PubSub {
  ...
  static getChannel(executionId: string): PubSubChannel {
    ...
    Logging.log(
      "PubSub",
      `Created channel for execution ${executionId} (total: ${PubSub.channels.size})`,
    );
  ...
  private static performChannelCleanup(executionId: string): void {
    ...
    Logging.log(
      "PubSub",
      `Deleted channel for execution ${executionId} (remaining: ${PubSub.channels.size})`,
    );
  }
  ...
  private static scheduleCleanup(executionId: string): void {
    ...
    const timer = setTimeout(() => {
      Logging.log(
        "PubSub",
        `Auto-cleanup triggered for channel ${executionId}`,
      );
      PubSub.performChannelCleanup(executionId);
