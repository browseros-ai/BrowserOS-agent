...
describe("KlavisAPIClient", () => {
  ...
  it("tests that client throws error when API key is empty", async () => {
    ...
    await expect(
      emptyClient.getUserInstances("user123", "Nxtscape"),
    ).rejects.toThrow("Klavis API key not configured");
  });
  ...
  it("tests that API errors are handled properly", async () => {
    ...
    await expect(
      client.getUserInstances("user123", "Nxtscape"),
    ).rejects.toThrow("Klavis API error: 401 Unauthorized");
  });
});
