  ...
  async execute(_input: GetSelectedTabsInput): Promise<ToolOutput> {
    try {
      this.executionContext
        .getPubSub()
        .publishMessage(
          PubSub.createMessage(`Getting selected tabs`, "thinking"),
        );
      ...
      return toolError(
        `Failed to get tab information: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
}
...
export function createGetSelectedTabsTool(
  ...
  return new DynamicStructuredTool({
    ...
    description:
      "Get information about currently selected tabs. Returns an array of tab objects with id, url, and title. If no tabs are selected, returns the current tab.",
