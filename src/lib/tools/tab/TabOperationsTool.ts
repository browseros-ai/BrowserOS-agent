...
export class TabOperationsTool {
  ...
  private async _listTabs(): Promise<ToolOutput> {
    try {
      const currentWindow =
        await this.executionContext.browserContext.getCurrentWindow();
  ...
  private async _listAllTabs(): Promise<ToolOutput> {
    try {
      ...
      return toolError(
        `Failed to list all tabs: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  private async _createNewTab(): Promise<ToolOutput> {
    try {
      const page =
        await this.executionContext.browserContext.openTab(DEFAULT_TAB_URL);
      ...
      this.executionContext
        ...
        .publishMessage(
          PubSub.createMessage(
            `Created new tab with ID: ${page.tabId}`,
            "thinking",
          ),
        );
  ...
  private async _switchToTab(tabIds?: number[]): Promise<ToolOutput> {
    ...
    try {
      ...
      return toolSuccess(
        `Switched to tab: ${tab.title || "Untitled"} (ID: ${tabId})`,
      );
    } catch (error) {
      return toolError(
    ...
    try {
      ...
      if (errors.length > 0) {
        ...
        this.executionContext
          ...
          .publishMessage(
            PubSub.createMessage(
              `Closed ${closedCount} tab(s), failed ${errors.length} tab(s)`,
              "thinking",
            ),
          );
        return toolSuccess(
          `Closed ${closedCount} tab(s). Failed to close ${errors.length} tab(s): ${errors.join(", ")}`,
        );
      }
      ...
      return toolError(
        `Failed to close tabs: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
}
...
export function createTabOperationsTool(
  ...
  return new DynamicStructuredTool({
    ...
    description:
      "Manage browser tabs: list tabs in current window (list), list all tabs (list_all), create new tab (new), switch to tab (switch), or close tabs (close). Use tabIds array for switch/close operations.",
