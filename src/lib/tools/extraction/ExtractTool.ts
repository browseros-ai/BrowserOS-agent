...
import {
  generateExtractorSystemPrompt,
  generateExtractorTaskPrompt,
} from "./ExtractTool.prompt";
...
export function createExtractTool(
  ...
  return new DynamicStructuredTool({
    name: "extract_tool",
    description:
      "Extract specific information from a web page using AI. Supports extracting text or links based on a task description.",
    ...
    func: async (args: ExtractInput): Promise<string> => {
      try {
        executionContext
          ...
          .publishMessage(
            PubSub.createMessage(
              `Extracting information from page ${args.tab_id}`,
              "thinking",
            ),
          );
        // Get the page for the specified tab
        const pages = await executionContext.browserContext.getPages([
          args.tab_id,
        ]);
        ...
        if (args.extract_type === "text") {
          ...
          rawContent =
            textSnapshot.sections && textSnapshot.sections.length > 0
              ? textSnapshot.sections
                  .map(
                    (section: any) =>
                      section.content ||
                      section.text ||
          ...
          // Convert sections to readable content
          rawContent =
            linksSnapshot.sections && linksSnapshot.sections.length > 0
              ? linksSnapshot.sections
                  .map(
                    (section: any) =>
                      section.content ||
                      section.text ||
        ...
        executionContext
          ...
          .publishMessage(
            PubSub.createMessage(
              `Extracted ${args.extract_type} from page ${title}, generated summary...`,
              "thinking",
            ),
          );
