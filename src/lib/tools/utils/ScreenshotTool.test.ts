...
describe("ScreenshotTool-unit-test", () => {
  ...
  it("tests that screenshot capture methods are called correctly", async () => {
    ...
    const mockPage = {
      takeScreenshot: vi
        .fn()
        .mockResolvedValue("data:image/jpeg;base64,base64imagedata123"),
    } as any;
    ...
    // Verify method calls
    expect(
      mockExecutionContext.browserContext.getCurrentPage,
    ).toHaveBeenCalled();
    ...
    expect(outputData.screenshot).toBe(
      "data:image/jpeg;base64,base64imagedata123",
    );
  });
