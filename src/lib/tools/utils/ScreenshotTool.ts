    ...
    func: async (args: ScreenshotToolInput): Promise<string> => {
      try {
        ...
        if (!page) {
          Logging.log(
            "ScreenshotTool",
            "No active page found to take screenshot",
            "error",
          );
          executionContext.messageManager.addAI(
            "Screenshot unavailable - no active page. Continuing without visual verification.",
          );
          return JSON.stringify(
            toolSuccess(
              "Screenshot unavailable. Proceeding without visual capture.",
            ),
          );
        }
        ...
        if (!screenshotDataUrl) {
          Logging.log(
            "ScreenshotTool",
            "Failed to capture screenshot - no data returned",
            "error",
          );
          executionContext.messageManager.addAI(
            "Screenshot capture failed. Continuing without visual verification.",
          );
          return JSON.stringify(
            toolSuccess(
              "Screenshot unavailable. Proceeding without visual capture.",
        ...
        Logging.log(
          "ScreenshotTool",
          `Error capturing screenshot: ${errorMessage}`,
          "error",
        );
        executionContext.messageManager.addAI(
          "Screenshot capture failed. Continuing without visual verification.",
