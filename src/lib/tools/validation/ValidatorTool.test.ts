...
describe("ValidatorTool-unit-test", () => {
  ...
  beforeEach(() => {
    ...
    mockPage = {
      takeScreenshot: vi
        .fn()
        .mockResolvedValue("data:image/jpeg;base64,mockScreenshotBase64String"),
    };

    // Mock browser context methods
    mockBrowserContext = {
      getBrowserStateString: vi
        .fn()
        .mockResolvedValue(
    ...
    mockLLM = {
      withStructuredOutput: vi.fn().mockReturnValue({
        invoke: vi.fn().mockResolvedValue({
          ...
          reasoning:
            "Task completed successfully. The submit button was clicked and confirmation page is displayed.",
          confidence: "high",
          suggestions: [],
        }),
      }),
  ...
  it("tests that validation captures screenshot and includes it in prompt", async () => {
    ...
    expect(humanMessageContent).toContain(
      "data:image/jpeg;base64,mockScreenshotBase64String",
    );
  ...
  it("tests that validation handles screenshot capture errors gracefully", async () => {
    ...
    mockPage.takeScreenshot.mockRejectedValue(
  ...
  it.skipIf(!hasApiKey)(
    ...
    async () => {
      ...
      const getConfigSpy = vi
        .spyOn(browserContext, "getConfig")
        .mockReturnValue({ useVision: true } as any);
      const getCurrentPageSpy = vi
        .spyOn(browserContext, "getCurrentPage")
        .mockResolvedValue(mockPage as any);
      const getBrowserStateStringSpy = vi.spyOn(
        browserContext,
...
[2] Gift message textbox
      ...
      const suggestsCheckout = validationData.suggestions.some(
        (s: string) =>
      ...
      console.log(
        "✅ Integration test passed - ValidatorTool correctly identifies incomplete order",
      );
