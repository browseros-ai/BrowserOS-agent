...
import {
  ...
  generateValidatorTaskPrompt,
} from "./ValidatorTool.prompt";
...
async function _chunkedValidation(
  ...
  Logging.log(
    "ValidatorTool",
    `Browser state too large, validating across ${totalChunks} chunks`,
    "info",
  );
...
export function createValidatorTool(
  ...
  return new DynamicStructuredTool({
    name: "validator_tool",
    description:
      "Validate if the task has been completed based on current browser state",
    ...
    func: async (args: ValidatorInput): Promise<string> => {
      try {
        executionContext
          ...
          .publishMessage(
            PubSub.createMessage(
              `Validating if the task is complete`,
              "thinking",
            ),
          );
        // Get LLM instance
        ...
        if (config.useVision && maxTokens >= MIN_TOKENS_FOR_USING_VISION) {
          ...
          config.useVision &&
          maxTokens < MIN_TOKENS_FOR_USING_VISION
        ) {
          Logging.log(
            "ValidatorTool",
            `Skipping vision - model token limit (${maxTokens}) is below ${MIN_TOKENS_FOR_USING_VISION}`,
            "info",
          );
        }
        ...
        const readOnlyMessageManager = new MessageManagerReadOnly(
        ...
        // Emit status message
        const status = validationData.isComplete
          ? `Task completed!`
          : `Task is incomplete, will continue execution...`;
        executionContext
          .getPubSub()
          .publishMessage(PubSub.createMessage(status, "thinking"));
