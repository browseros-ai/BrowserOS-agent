...
const MCPToolInputSchema = z.object({
  ...
  instanceId: z
    .string()
    .optional()
    .describe("Instance ID for listTools and callTool"),
...
export class MCPTool {
  ...
  async execute(input: MCPToolInput): Promise<ToolOutput> {
    try {
      ...
      return toolError(
        `MCP operation failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  private async _getUserInstances(): Promise<ToolOutput> {
    try {
      ...
      if (instances.length === 0) {
        return toolSuccess(
          JSON.stringify({
            instances: [],
            message:
              "No MCP servers installed. Please install servers in Settings > Integrations.",
          }),
        );
      }
      ...
      instances.forEach((instance) => {
        this.instancesCache.set(instance.id, {
          id: instance.id,
          name: instance.name,
        });
      });
      ...
      return toolError(
        `Failed to get user instances: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  private async _listTools(instanceId?: string): Promise<ToolOutput> {
    ...
    if (!instance) {
      return toolError(
        `Instance ${instanceId} not found. Please run getUserInstances first.`,
      );
    }
