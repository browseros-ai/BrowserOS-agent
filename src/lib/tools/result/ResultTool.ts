...
import {
  ...
  generateResultTaskPrompt,
} from "./ResultTool.prompt";
    ...
    func: async (args: ResultInput): Promise<string> => {
      try {
        ...
        executionContext
          ...
          .publishMessage(
            PubSub.createMessage(
              `Generating result for task: ${args.task} after execution`,
              "thinking",
            ),
          );
        ...
        const result = await invokeWithRetry<
          z.infer<typeof ResultSummarySchema>
        >(structuredLLM, messages, 3, { signal: executionContext.abortSignal });

        executionContext
          .getPubSub()
          .publishMessage(
            PubSub.createMessage(
              `Generated result for task: ${args.task}`,
              "thinking",
            ),
          );
        ...
        executionContext
          ...
          .publishMessage(
            PubSub.createMessageWithId(
              ...
              `Result generation failed: ${errorMessage}`,
              "error",
            ),
          );
