...
describe("TodoManagerTool", () => {
  ...
  it("tests that list action returns XML format", async () => {
    ...
    expect(parsed.output).toContain(
      '<todo id="1" status="todo">First task</todo>',
    );
    expect(parsed.output).toContain(
      '<todo id="2" status="todo">Second task</todo>',
    );
  });
  ...
  it("tests that skip action validates single ID only", async () => {
    ...
    expect(multipleIdsParsed.output).toContain(
      "skip action requires exactly one ID",
    );
