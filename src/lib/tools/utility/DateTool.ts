...
const DateToolInputSchema = z.object({
  ...
  dayStart: z
    ...
    .optional()
    .describe(
      "For custom range: days back from today for START (e.g., 300 = 300 days ago)",
    ),
  ...
  dayEnd: z
    ...
    .optional()
    .describe(
      "For custom range: days back from today for END (e.g., 5 = 5 days ago)",
    ),

  format: z
    .enum(["iso", "date", "us", "eu", "unix"])
    .default("date")
    .describe(
      "Output format: iso (ISO-8601), date (YYYY-MM-DD), us (MM/DD/YYYY), eu (DD/MM/YYYY), unix (timestamp)",
    ),
});
...
export class DateTool {
  ...
  async execute(input: DateToolInput): Promise<ToolOutput> {
    try {
      ...
      switch (date_range) {
        ...
        case "custom":
          ...
          if (dayStart === undefined || dayEnd === undefined) {
            return toolError(
              "dayStart and dayEnd are required for custom date range",
            );
          }
          if (dayStart < dayEnd) {
            return toolError(
              "dayStart must be >= dayEnd (dayStart is further back in time)",
            );
          }
      ...
      return toolError(
        `Date calculation failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
