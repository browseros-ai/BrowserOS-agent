  ...
  async execute(input: FindElementInput): Promise<ToolOutput> {
    try {
      this.executionContext
        .getPubSub()
        .publishMessage(PubSub.createMessage(`Finding element...`, "thinking"));
      // Get browser state
      const browserState =
      ...
      if (!result.found || result.index === null) {
        return toolError(
          result.reasoning ||
            `No element found matching "${input.elementDescription}"`,
        );
      }
      ...
      const foundInClickable = browserState.clickableElements.find(
        (el) => el.nodeId === result.index,
      );
      const foundInTypeable = browserState.typeableElements.find(
        (el) => el.nodeId === result.index,
      );

      if (!foundInClickable && !foundInTypeable) {
        return toolError(
          `Invalid index ${result.index} returned - element not found`,
        );
      }
      ...
      this.executionContext
        ...
        .publishMessage(
          PubSub.createMessage(
            `Found element at index: ${result.index}`,
            "thinking",
          ),
        );
...
export function createFindElementTool(
  ...
  return new DynamicStructuredTool({
    ...
    description:
      "Find an element on the page using a natural language description. Returns found (boolean), index (number), confidence level, and reasoning.",
