...
const _FindElementSchema = z.object({
  ...
  index: z
    ...
    .nullable()
    .describe(
      "The index number of the best matching element (null if not found)",
    ),
  confidence: z
    .enum(["high", "medium", "low"])
    .nullable()
    .describe("Confidence level in the match (null if not found)"),
...
export class InteractionTool {
    ...
    switch (input.operationType) {
      ...
      case "input_text":
        if (!input.description || !input.input_text) {
          return toolError(
            "input_text operation requires description and input_text parameters",
          );
        }
        return await this._inputTextElement(
          input.description,
          input.input_text,
        );
  ...
  private async _findElementWithLLM(
    description: string,
  ): Promise<z.infer<typeof _FindElementSchema>> {
    // Get max tokens from MessageManager (which gets it from model capabilities)
    const _MAX_TOKENS_PER_CHUNK =
      this.executionContext.messageManager.getMaxTokens();
    ...
    // Get browser state
    const browserState =
      await this.executionContext.browserContext.getBrowserState();
    if (
      !browserState.clickableElements.length &&
      !browserState.typeableElements.length
    ) {
    ...
    const browserStateString =
      await this.executionContext.browserContext.getBrowserStateString(true); // simplified=true
    const chunker = new BrowserStateChunker(
      browserStateString,
    ...
    Logging.log(
      "InteractionTool",
      `Searching through ${totalChunks} chunks for element`,
      "info",
    );
    for (let i = 0; i < totalChunks; i++) {
      ...
      Logging.log(
        "InteractionTool",
        `Searching chunk ${i + 1}/${totalChunks} with ${TokenCounter.format(tokenCount)}`,
        "info",
      );
      ...
      if (result.found && result.index !== null) {
        Logging.log(
          "InteractionTool",
          `Element found in chunk ${i + 1}/${totalChunks}`,
          "info",
        );
        return result;
      }
  ...
  private async _findElement(
    ...
    if (!result.found || result.index === null) {
      throw new Error(
        result.reasoning || `No element found matching "${description}"`,
      );
    }
    ...
    const elements = isClickable
      ? browserState.clickableElements
      : browserState.typeableElements;

    const found = elements.find((el) => el.nodeId === result.index);

    if (!found) {
      throw new Error(
        `Invalid index ${result.index} returned - element not found or wrong type for ${interactionType}`,
      );
    }
  ...
  private async _clickElement(description: string): Promise<ToolOutput> {
    this.executionContext
      ...
      .publishMessage(
        PubSub.createMessage(
          `Finding element to click with description: ${description}`,
          "thinking",
        ),
      );
    ...
    for (let attempt = 1; attempt <= NUM_RETRIES; attempt++) {
      try {
        ...
        if (page.isFileUploader(element)) {
          return toolError(
            `Element "${description}" opens a file upload dialog. File uploads are not supported.`,
          );
        }
        ...
        await new Promise((resolve) =>
          setTimeout(resolve, INTERACTION_WAIT_MS),
        );
    ...
    return toolError(
      `Failed to click "${description}" after ${NUM_RETRIES} attempts`,
    );
  }

  // Input text with retry logic
  private async _inputTextElement(
    description: string,
    text: string,
  ): Promise<ToolOutput> {
    this.executionContext
      ...
      .publishMessage(
        PubSub.createMessage(
          `Finding element to type into with description: ${description}`,
          "thinking",
        ),
      );
    ...
    return toolError(
      `Failed to input text into "${description}" after ${NUM_RETRIES} attempts`,
    );
  }
  ...
  private async _clearElement(description: string): Promise<ToolOutput> {
    for (let attempt = 1; attempt <= NUM_RETRIES; attempt++) {
      try {
        this.executionContext
          ...
          .publishMessage(
            PubSub.createMessage(
              `Finding element to clear with description: ${description}`,
              "thinking",
            ),
          );
        // Find element (returns nodeId)
        ...
        this.executionContext
          .getPubSub()
          .publishMessage(
            PubSub.createMessage(`Cleared: ${description}`, "thinking"),
          );
    ...
    return toolError(
      `Failed to clear "${description}" after ${NUM_RETRIES} attempts`,
    );
  }
