  ...
  private async _navigateToUrl(url?: string): Promise<ToolOutput> {
    ...
    try {
      this.executionContext
        .getPubSub()
        .publishMessage(
          PubSub.createMessage(`Navigating to: ${url}`, "thinking"),
        );
      ...
      if (errorMessage.includes("not allowed")) {
        return toolError(
          `URL not allowed: ${url}. This URL is restricted by security policy.`,
        );
      }
  ...
  private async _goBack(): Promise<ToolOutput> {
    try {
      ...
      this.executionContext
        .getPubSub()
        .publishMessage(
          PubSub.createMessage(`Went back to: ${currentUrl}`, "thinking"),
        );
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      if (
        errorMessage.includes("Cannot navigate back") ||
        errorMessage.includes("no previous page")
      ) {
  ...
  private async _goForward(): Promise<ToolOutput> {
    try {
      ...
      this.executionContext
        .getPubSub()
        .publishMessage(
          PubSub.createMessage(`Went forward to: ${currentUrl}`, "thinking"),
        );
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      if (
        errorMessage.includes("Cannot navigate forward") ||
        errorMessage.includes("no next page")
      ) {
  ...
  private async _refresh(): Promise<ToolOutput> {
    try {
      ...
      this.executionContext
        .getPubSub()
        .publishMessage(
          PubSub.createMessage(`Refreshed page: ${currentUrl}`, "thinking"),
        );
...
export function createNavigationTool(
  ...
  return new DynamicStructuredTool({
    ...
    description:
      "Navigate the browser: go to URL, go back/forward in history, or refresh the page",
