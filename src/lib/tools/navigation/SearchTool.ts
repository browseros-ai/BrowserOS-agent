      ...
      this.executionContext
        ...
        .publishMessage(
          PubSub.createMessage(
            `Searched for "${input.query}" on ${this._getProviderName(input.searchProvider)}`,
            "thinking",
          ),
        );

      return toolSuccess(
        `Searched for "${input.query}" on ${input.searchProvider}`,
      );
    } catch (error) {
      return toolError(
...
export function createSearchTool(
  ...
  return new DynamicStructuredTool({
    name: "search_tool",
    description:
      "Perform searches on different platforms: google (web search), amazon (products), google_maps (locations), google_finance (stocks).",
