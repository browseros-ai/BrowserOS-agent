...
describe("ScrollTool", () => {
  ...
  it("tests that scroll_to_element validates index requirement", async () => {
    ...
    expect(result.output).toBe(
      "scroll_to_element operation requires index parameter",
    );
  });
  ...
  it("tests that scroll operations execute correctly", async () => {
    ...
    const mockPage = {
      ...
      getElementByIndex: vi
        .fn()
        .mockResolvedValue({ nodeId: 42, tag: "button", text: "Submit" }),
      scrollToElement: vi.fn().mockResolvedValue(true),
    };
    vi.spyOn(browserContext, "getCurrentPage").mockResolvedValue(
      mockPage as any,
    );
