...
export class ScrollTool {
  ...
  async execute(input: ScrollInput): Promise<ToolOutput> {
    // Validate input
    if (
      input.operationType === "scroll_to_element" &&
    ...
    try {
      ...
      return toolError(
        `Scroll operation failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  private async _scrollDown(page: any, times?: number): Promise<ToolOutput> {
    const count =
      typeof times === "number" && times > 0 ? times : DEFAULT_VIEWPORT_COUNT;
    ...
    this.executionContext
      .getPubSub()
      .publishMessage(PubSub.createMessage(msg, "thinking"));
  ...
  private async _scrollToElement(
    page: any,
    index: number,
  ): Promise<ToolOutput> {
    ...
    const elementInfo =
      `${element.tag || "unknown"} "${element.text || ""}"`.trim();

    // Emit status message
    this.executionContext
      .getPubSub()
      .publishMessage(
        PubSub.createMessage(`Scrolled to element: ${elementInfo}`, "thinking"),
      );
...
export function createScrollTool(
  ...
  return new DynamicStructuredTool({
    name: "scroll_tool",
    description:
      "Perform scrolling operations: scroll_down/up (by viewports) or scroll_to_element (by index). Optional 'times' controls how many viewports to scroll (default 1).",
