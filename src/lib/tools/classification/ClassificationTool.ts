...
export class ClassificationTool {
  ...
  async execute(input: ClassificationInput): Promise<string> {
    try {
      ...
      const reader = new MessageManagerReadOnly(
      ...
      this.executionContext
        ...
        .publishMessage(
          PubSub.createMessage(
            `Task classified as ${result.is_simple_task ? "simple" : "complex"} and ${result.is_followup_task ? "follow-up" : "new"}`,
            "thinking",
          ),
        );
...
export function createClassificationTool(
  ...
  const classificationTool = new ClassificationTool(
    executionContext,
    toolDescriptions,
  );
