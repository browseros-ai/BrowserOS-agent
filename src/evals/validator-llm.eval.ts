...
} from "@/lib/tools/validation/ValidatorTool.prompt";
...
async function runValidatorLLMEvaluation() {
  ...
  for (let i = 0; i < testCases.length; i++) {
    ...
    try {
      ...
      console.log(
        `  Result: ${validation.validation.isComplete ? "Complete" : "Incomplete"}`,
      );
  ...
  const accurateValidations = results.filter(
    (r) => r.actualResult === r.expectedResult,
  ).length;
      ...
      async (input: z.infer<typeof ValidatorTestCaseSchema>, output: any) => {
        ...
        return {
          ...
          metadata: {
            ...
            accurateValidation:
              output.result.isComplete === input.expected.isComplete,
          },
        };
      },
    ],
